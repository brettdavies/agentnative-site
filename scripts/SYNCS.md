...
flowchart LR
    ...
    spec -- "scripts/sync-prose-tooling.sh<br/>(remote-first, manual)" --> site
...
| `brettdavies/agentnative` (spec) prose-check tooling: `BRAND.md`, `styles/brand/*.yml` + `README.md`, `styles/config/vocabularies/brand/{accept,reject}.txt`, `scripts/prose-check.sh`, `scripts/generate-pack-readme.mjs`                           | `scripts/sync-prose-tooling.sh` (manual; same remote-first / local-fallback / latest-v*-tag pattern as `sync-spec.sh`; extracts via `git show "$tag:<path>" >dest`; restores executable bit on `prose-check.sh`) | → repo-rooted: `BRAND.md`, `styles/brand/`, `styles/config/vocabularies/brand/`, `scripts/prose-check.sh`, `scripts/generate-pack-readme.mjs` | After a spec release that touches the prose-check stack. Separate sync clock from `sync-spec.sh` because prose tooling and the principles/contract release on different cadences. | None automated on this side. Sync-script atomicity is the integrity guarantee: brand `*.yml` AND its `README.md` come from the same tag, so no downstream regeneration / drift surface. `git diff` after sync is the review surface. Idempotent at a fixed spec tag. **Site-local divergence:** `scripts/prose-check.sh` carries two consumer-side carveouts not yet supported upstream — nested `*/node_modules/*` and `src/data/spec/` + `content/principles/` exclusions. The file's `SITE-LOCAL DIVERGENCE` header marker documents the patch and points at the upstream todo (`agentnative-spec/.context/compound-engineering/todos/010-pending-p0-prose-check-consumer-exclusion-config.md`). Re-apply after each `sync-prose-tooling.sh` run; `git diff scripts/prose-check.sh` post-sync surfaces the regression. |
...
4. **Spec cuts a tag that touches prose tooling** → maintainer runs `bash scripts/sync-prose-tooling.sh` (same tag and
   resolution pattern as `sync-spec.sh`) → vendored `BRAND.md`, `styles/brand/`, `styles/config/vocabularies/brand/`,
   `scripts/prose-check.sh`, and `scripts/generate-pack-readme.mjs` update in place. Separate sync clock from item 3
   because prose tooling and the principles/contract release on different cadences — a tag may touch one, the other, or
   both. Brand pack `*.yml` AND its companion `README.md` come from the same tag (no downstream regeneration of the
   brand README, which would invite tooling-version drift). Same `repository_dispatch:spec-release` event applies; same
   deferred consumer-side auto-PR handler.
...
- `scripts/sync-prose-tooling.sh` — header comment for the prose-check vendor manifest and rationale (separate sync
  clock from `sync-spec.sh`; brand README is a released artifact, not regenerated downstream). Shares `SPEC_REMOTE_URL`
  / `SPEC_ROOT` env vars with `sync-spec.sh`.
