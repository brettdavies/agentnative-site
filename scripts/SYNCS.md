...
flowchart LR
    subgraph In ["Inbound"]
        ...
        cliVersion[brettdavies/agentnative-cli<br/>Cargo.toml @ pinned tag]
    ...
    cliVersion -- "scripts/sync-cli-version.sh<br/>(gh api, manual, latest v* tag<br/>or --ref &lt;branch|tag|SHA&gt;)" --> site
...
4. **CLI cuts a new tag (binary release)** → maintainer runs `bash scripts/sync-cli-version.sh` (auto-picks the latest
   ...
   updates → `bun run build` regenerates `src/worker/spec-version.gen.ts` so `ANC_VERSION` reflects the new release.
   Test fixtures that mock `anc_version` (cache payloads, response triads, `anc --version` stdout) import `ANC_VERSION`
   and pick up the new value automatically; the red-team meta-test in `tests/spec-version-hardcoding.test.ts` blocks any
   fixture that drifts back to a hardcoded literal. For cross-repo coordination of an in-flight cli release that hasn't
   tagged yet, pass `--ref dev` (or a SHA, or set `CLI_REF=<ref>`) and record the resolved short SHA in the consumer PR
   body.
...
- `scripts/sync-spec.sh`: header comment for usage; `--ref <branch|tag|SHA>` flag and `SPEC_REF` env var for vendoring
  ...
  offline use.
- `scripts/sync-cli-version.sh`: header comment for usage; `--ref` flag and `CLI_REF` env var for vendoring explicit
