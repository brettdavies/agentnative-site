...
flowchart LR
    ...
    spec -- "scripts/sync-spec.sh<br/>(remote-first, manual, latest v* tag)" --> site
...
4. **Spec's `main` advances with prose-tooling changes** → maintainer runs `bash scripts/sync-prose-tooling.sh` (same
   ...
   `styles/brand/`, `styles/config/vocabularies/brand/`, `scripts/prose-check.sh`, and
   `scripts/generate-pack-readme.mjs` update in place. Separate sync clock from item 3 because prose tooling and the
   principles/contract release on different cadences. Brand pack `*.yml` AND its companion `README.md` come from the
   same `main` HEAD SHA (no downstream regeneration of the brand README, which would invite tooling-version drift).
