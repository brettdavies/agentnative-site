...
case "$ENV" in
  local | staging) ;;
  *)
    echo "invalid --env: $ENV (must be 'local' or 'staging')" >&2
    exit 2
    ;;
esac
...
read_op_field() {
  [[ -x "$OP_SKILL/read_field.sh" ]] || {
    echo "1Password skill not found at $OP_SKILL" >&2
    return 2
  }
...
gate_mechanics() {
  ...
  # Both screens below match against the guarded set the workflow enforces,
  # resolved from it rather than restated here. A restated copy drifts, and a
  # copy that omits a guarded path reports a leak as clean.
  local guarded
  ...
  if git rev-parse --verify origin/main >/dev/null 2>&1; then
    ...
    if [[ -z "$added_docs" ]]; then
      ...
      gate_skip "unguarded docs added to main (confirm each is meant to ship)" \
        "$(echo "$added_docs" | tr '\n' ' ')"
    fi
  fi
