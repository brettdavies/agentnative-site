...
//
// `src/build/00-spec-version-gen.mjs` regenerates the file from three
// vendored markers: `src/data/spec/VERSION`, `content/principles/VERSION`,
// and `src/data/anc/VERSION`. The build itself runs the emitter, but the
// .gen.ts file is committed so the worker bundle can be type-checked
// without a build step. This test re-runs the emitter in memory and
// asserts the on-disk file matches — so an out-of-date committed file
// fails CI rather than silently shipping a stale triad.
    ...
    if (actual !== expected.content) {
      throw new Error(
        `src/worker/spec-version.gen.ts is out of date relative to VERSION files. ` +
          ...
          `ANC_VERSION=${expected.ancVersion}.`,
      );
    }
    expect(actual).toBe(expected.content);
  });

  test('renderer emits all four constants when given the full input set', async () => {
    ...
    expect(content).toContain("export const ANC_VERSION = '4.5.6'");
