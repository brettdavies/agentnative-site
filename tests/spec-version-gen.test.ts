...
//
// `src/build/spec-version-gen.mjs` regenerates the file from
...
import { computeExpectedSpecVersionModule } from '../src/build/spec-version-gen.mjs';
...
describe('spec-version-gen drift guard', () => {
    ...
    if (actual !== expected.content) {
      throw new Error(
        `src/worker/spec-version.gen.ts is out of date relative to VERSION files. ` +
          `Run \`bun run build\` (or \`bun src/build/spec-version-gen.mjs\`) and commit the result. ` +
  ...
  test('emitter rejects empty VERSION input', async () => {
    ...
    const { renderSpecVersionModule } = await import('../src/build/spec-version-gen.mjs');
    ...
    expect(content).toContain('GENERATED by src/build/spec-version-gen.mjs');
  });
});
