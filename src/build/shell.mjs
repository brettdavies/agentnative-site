...
// Grouped primary nav. Each entry covers a route group; `match` lists the
...
const AI_PROVIDERS = [
  ...
  },
  {
    ...
    svg: '<svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M17.3041 3.541h-3.6718l6.696 16.918H24zm-10.6082 0L0 20.459h3.7442l1.3693-3.5527h7.0052l1.3693 3.5528h3.7442L10.5363 3.5409zm-.3712 10.2232 2.2914-5.9456 2.2914 5.9456z"/></svg>',
  },
  {
    name: 'Gemini',
  ...
  },
  {
    ...
    svg: '<svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M14.234 10.162 22.977 0h-2.072l-7.591 8.824L7.251 0H.258l9.168 13.343L.258 24H2.33l8.016-9.318L16.749 24h6.993zm-2.837 3.299-.929-1.329L3.076 1.56h3.182l5.965 8.532.929 1.329 7.754 11.09h-3.182z"/></svg>',
  },
  {
    name: 'Perplexity',
...
${SOURCE_REPOS.map(
  (r, i) =>
    `          ${i > 0 ? '<span class="site-footer__sep" aria-hidden="true">·</span>\n          ' : ''}<a href="${r.url}" rel="noopener noreferrer">${r.name}</a>`,
).join('\n')}
        </p>
        <p class="site-footer__meta">
          <span>${SITE_NAME} · v${SITE_SPEC_VERSION}</span>
