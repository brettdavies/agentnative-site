...
// `AUDITOR_URL`; `response-shape.ts` consumes it for every response triad.
//
 ...
 */
export function renderSpecVersionModule({ specVersion, siteSpecVersion, auditorUrl }) {
...
export async function generateSpecVersionModule() {
  ...
  const content = renderSpecVersionModule({ specVersion, siteSpecVersion, auditorUrl: AUDITOR_URL });
  await writeFile(GEN_PATH, content);
  return { specVersion, siteSpecVersion, auditorUrl: AUDITOR_URL, content, path: GEN_PATH };
}
...
export async function computeExpectedSpecVersionModule() {
  ...
  return {
    ...
    content: renderSpecVersionModule({ specVersion, siteSpecVersion, auditorUrl: AUDITOR_URL }),
    path: GEN_PATH,
  };
}
