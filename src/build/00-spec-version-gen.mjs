...
//
// Reads three vendored markers and writes a TS module the Worker imports
// at build time. The emitted file is the single source of truth for
// `SPEC_VERSION`, `SITE_SPEC_VERSION`, `ANC_VERSION`, and `AUDITOR_URL`;
// `response-shape.ts` consumes the version triad for every response.
//
// Three-file split is load-bearing: spec VERSION, site-principles VERSION,
// and anc VERSION are released independently. A spec bump can ship before
// the site copy updates; an anc bump can ship without a spec change.
// Rendering all three makes lag observable on every response.
//
...
// and asserts the committed `.gen.ts` matches — so an out-of-date
// generated file fails CI instead of silently shipping a stale triad.
 ...
 */
export function renderSpecVersionModule({ specVersion, siteSpecVersion, ancVersion, auditorUrl }) {
...
export const ANC_VERSION = '${ancVersion}';
export const AUDITOR_URL = '${auditorUrl}';
`;
}
...
export async function generateSpecVersionModule() {
  ...
  const ancVersion = await readVersion(ANC_VERSION_PATH);
  const content = renderSpecVersionModule({
    specVersion,
    siteSpecVersion,
    ancVersion,
    auditorUrl: AUDITOR_URL,
  });
...
export async function computeExpectedSpecVersionModule() {
  ...
  return {
    ...
    ancVersion,
    auditorUrl: AUDITOR_URL,
    content: renderSpecVersionModule({
      specVersion,
      siteSpecVersion,
      ancVersion,
      auditorUrl: AUDITOR_URL,
