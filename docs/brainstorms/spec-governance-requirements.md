...
The agent-native CLI standard spans two public repos (spec site, checker tool) with no formal governance model. Issues
...
- `agentnative-cli` — the checker tool (GitHub rename only, crate name unchanged)
- `agentnative-site` — the website (unchanged, consumes spec repo as dependency)

1. **Coupled releases.** A principle revision is only complete when the corresponding checker is also reviewed and
...
- R1.6: README links to anc.dev for the rendered spec and to the tool repo for the checker.
...
- R3.1: Issue templates scoped to checker concerns: false positives/negatives, CLI feature requests, scoring bugs.
...
- R5.3: `anc check --output json` includes the check's revision date in the output.
...
- A tool consumer can answer "which version of P3 was I scored against?" from the `anc check` output alone.
