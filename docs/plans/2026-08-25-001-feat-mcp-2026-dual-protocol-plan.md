...
- **Stop conditions:** Stop and surface if `agents/mcp/server` `createMcpHandler` cannot pass through SDK `legacy`
  options on the pinned `agents` version; if upgrading breaks the Worker bundle size budget materially (record `wrangler
  deploy --dry-run` delta against `dev` baseline); or if dual-stack conformance tests cannot run without a network fetch
  (use in-process `handler.fetch` per SDK migration guide).
...
current specification: stateless by default, no `initialize` handshake on the modern lane, cache hints on list/read, and
routable `Mcp-Method` / `Mcp-Name` headers. This work upgrades the server to SDK v2 while preserving legacy clients
...
progress notifications on audits), which matches the new spec's direction. The gap is version lag (declared
`2025-06-18`, v1 imports, per-request handler rebuild workaround) and missing observability to retire legacy safely.
Reimplementing protocol routing in site code would duplicate `@modelcontextprotocol/server` and drift from Cloudflare's
maintained wrapper. The fix is retarget imports, add a thin shell for product-specific gates and logs, and bump the
drift-gated spec literals in lockstep.
...
- **KD3. Module-scoped handler singleton with catalog pre-warm.** `handleMcp` awaits `loadCatalog(env)` before dispatch;
  the SDK factory reads the isolate cache synchronously. Replaces today's per-request `buildMcpHandler` rebuild. Governs
  KTD3, U1.
...
- **KD7. Dual-stack smoke stays in `scripts/release/mcp-smoke.sh`; not gated off `main`.** Validated pattern elsewhere
  gates staging-only smoke under `scripts/smoke/` via `guard-main-docs.yml` `extra_paths` so it never ships on `main`.
  anc rejects that split: `preflight.sh mcp` and `postflight.sh --env prod mcp` invoke `scripts/release/mcp-smoke.sh`
  against **prod** (`anc.dev`) as a release gate — the script must remain mergeable to `main`. Dual-stack checks 1–6
  land in the existing release script; do **not** add `scripts/release/` to `extra_paths`. Governs U5, release workflow.
...
- R1. MCP POST `/mcp` serves both legacy-era and modern-era clients on the same endpoint using SDK dual-stack with
  default `legacy: 'stateless'` unless `MCP_LEGACY_ENABLED` is `'false'`, in which case the handler uses `legacy:
  'reject'`.
...
- R3. Declared spec revision is `2026-07-28` in server instructions, server card seed
  (`src/build/11a-discovery-emit.mjs`), `content/mcp-skill.md`, `AGENTS.md` MCP section, and e2e card assertions. Legacy
  `initialize` responses continue to advertise compatibility per SDK dual-stack behavior.
...
- R6. Rate limiting:
  ...
  - **Audit tiers (tool handlers, both eras):** `score_cli` continues through `MCP_AUDIT_LIMITER` + KV hourly;
    `audit_website` continues through `WEB_AUDIT_LIMITER_IP` + KV hourly. Shell may use trusted `Mcp-Name` for telemetry
    classification only; it must not bypass tool-level audit gates.
...
- F4. **Legacy sunset staging flip.** A2 sets `MCP_LEGACY_ENABLED=false` → legacy `initialize` logs `outcome:
  legacy_rejected` at shell; modern request succeeds. **Covers R1, R8.**
...
- AE2. **When** modern client sends `tools/list` with required headers and full `_meta` (including `clientCapabilities`)
  **then** response includes `ttlMs` (expect `3600000`) and `cacheScope: public` on list results; tool names match
  legacy `tools/list`. **Covers R4, R10.**
...
**In scope:** R1–R12; SDK v2 dual-stack; shell telemetry; era-aware read-tier rate-limit keys; `cacheHints`; spec bump
on drift-gated surfaces; mcp-skill.md modern quick-reference section; runbook update; `scripts/release/mcp-smoke.sh`
...
- Pinned `agents@^0.21.0` passes `legacy` and `responseMode` through to SDK v2 (documented in
  `node_modules/agents/docs/mcp-servers.md`); `isLegacyRequest` is exported from `@modelcontextprotocol/server` and used
  by `agents/mcp/server`.
...
- Shipped baseline: `docs/runbooks/mcp-operator.md`, `content/mcp-skill.md`,
  `docs/plans/2026-06-05-001-feat-mcp-endpoint-plan.md` (completed).
...
assertion, bundle-size dry-run gate, and explicit no-CORS divergence from SDK browser allowlist options. No scope
change.
...
- KTD3. **Handler singleton (bounded cache).** Module-scoped `createMcpHandler` instances cached by `{ responseMode,
  ...
  `responseMode: 'auto'`. `handleMcp` awaits `loadCatalog(env)` before dispatch so the sync factory reads warmed
  catalog. Instantiates KD3.
- KTD4. **Cache hint values for static build-time catalog.** `tools/list`, `resources/list`, `resources/read`: `ttlMs:
  3_600_000`, `cacheScope: 'public'`. Values live in `ServerOptions.cacheHints`; SDK emits fields on modern responses
  only. Do not attach hints to dynamic scorecard or web-audit tool results. Instantiates R4.
...
- **Verification:** `bun run build && bun test tests/worker-mcp.test.ts tests/worker-mcp-audit.test.ts
  tests/worker-mcp-dispatch.test.ts`; typecheck clean; `wrangler deploy --dry-run` bundle size recorded and compared to
  `dev` baseline — fail review if delta is material without explicit acceptance.
...
- **Test scenarios:**
  ...
  - Rate limiter invoked with `legacy:{ip}` for legacy initialize; `modern:get_scorecard:{ip}` for modern `tools/call
    get_scorecard` when name is registered; spoofed `Mcp-Name: not_a_real_tool` falls back to `modern:{ip}`.
  - `MCP_LEGACY_ENABLED=false` + legacy `initialize` → shell logs `outcome: legacy_rejected` before SDK dispatch; modern
    direct `tools/call` on same handler still succeeds.
...
- **Approach:**
  1. Add helpers building POST bodies with `_meta` in **params** (`io.modelcontextprotocol/protocolVersion`,
     `clientInfo`, `clientCapabilities` — **required on both list and call**) and headers `MCP-Protocol-Version:
     2026-07-28`, `Mcp-Method`, plus `Mcp-Name` on call only.
...
- **Approach:**
  1. Replace spec revision pin section in runbook; document `MCP_LEGACY_ENABLED`, telemetry schema, example
     Observability filters (`era: legacy`, `% by era`, top-N `client_name`).
  ...
  6. **Do not** add `scripts/release/` to `.github/workflows/guard-main-docs.yml` `extra_paths` (KD7). That pattern
     applies when smoke is staging-only dev tooling under `scripts/smoke/`; anc's release script is load-bearing for
     prod postflight. If a future staging-only diagnostic is split out, place it under `scripts/smoke/` and add that
     path to `extra_paths` — out of scope for this unit.
...
- **SDK `allowedOriginHostnames` for browser CORS on `/mcp`.** Rejected: anc's deliberate no-CORS posture (runbook
  threat model); agent runtimes do not issue browser preflights. Host allowlists are for browser-reachable MCP, not this
  surface.
...
| `host`             | Request host                                                                                                                                 |
| `response_format`  | `json` \| `sse`, from the served `content-type` (`text/event-stream` is `sse`)                                                               |
...
- **Script shell:** `set -uo pipefail`; shared `report <num> <name> <ok|no> <detail>` printing `CHECK N [label]:
  PASS|FAIL …`; final `RESULT: N pass / M fail`; exit non-zero on any failure. Write each raw response to a `mktemp -d`
  dir (path printed on exit) for failure triage.
...
- **Headers:** modern list needs `MCP-Protocol-Version` + `Mcp-Method: tools/list` only; modern call adds `Mcp-Name:
  get_scorecard`. Legacy probes use plain `content-type: application/json` + `accept: application/json`. Anc-specific
  symmetry/live-audit extensions retain existing dual-MIME Accept (`application/json, text/event-stream`) and `-K` CF
  Access config file pattern.
- **Auth:** staging runs require CF Access (`CF_ACCESS_CLIENT_ID` / `CF_ACCESS_CLIENT_SECRET`); prod `anc.dev` runs omit
  auth headers. `MCP_SMOKE_BASE` env aliases positional `<base-url>` when set.
- **Parsing helpers:** extract cache hints with deep jq walk: `[.. | objects | .ttlMs? // empty] | first`; miss
  detection via inner envelope: `jq -r '.result.content[0].text' | jq -r '.found'`.
...
Validated pattern for staging-only smoke: add `scripts/smoke/` to `.github/workflows/guard-main-docs.yml` `extra_paths`
so the script never merges to `main` (sits outside deploy path filters; dev/staging operator tooling only). anc **does
not** follow this for the dual-stack smoke work (KD7):
