...
- **Objective:** An agent can inspect any website audit, identify every relevant finding, retrieve complete remediation prompts by normative priority and exact result status, and tell when every returned per-target audit becomes eligible to leave the cache-reuse window.
...
The audit cache already records `scored_at`, and the five-minute staleness threshold is authoritative. Most response surfaces discard that information. Agents therefore cannot distinguish a fresh result from a cache hit or know when a recheck becomes eligible to leave the cache-reuse window. Eligibility does not guarantee that a fresh audit can run; kill switches, rate limits, and service failures still apply.
...
- R6. `get_fix_prompt` returns structured data for one ID. It returns the complete prompt for an observed remediable row, an explicit reason for a known non-remediable row, and an explicit unknown-ID result when no rendered row has the requested ID.
...
- R19. Target-controlled audit evidence remains untrusted data. WebMCP remediation prompts do not interpolate it into instruction-bearing text; the rendered Result field remains the evidence surface.
- R20. All paginated finding surfaces order by keyword `must`, `should`, `may`; then status `broken`, `absent`, `noncompliant`, `error`, `pass`, `n_a`, `skip`; then observed before `unprobed`; then rendered document order.
- R21. Pagination defaults to `offset: 0` and `limit: 10`, accepts integer offsets of zero or greater and integer limits from 1 through 25, rejects present empty filter arrays, and returns `next_offset: null` when exhausted. `omitted` is the number of matching items after the returned page; output-cap packing may return fewer than `limit` but must advance by `returned` without gaps.
...
- R12. `refresh_after` is derived from the same authoritative `scored_at` instant plus `WEB_AUDIT_STALE_AFTER_MS`. It is never stored separately and means only the earliest cache-expiry eligibility time, not guaranteed fresh-audit availability.
...
- R22. Read-time registry enrichment hydrates `category`, `keyword`, and `tier` by stable check ID before either result renderer receives a cached scorecard; missing registry entries retain stored values.
- R23. HTML renders semantic scored and refresh timestamps near the result summary. Future eligibility reads “Refresh available after <time>,” expired eligibility reads “Refresh available now,” and a null legacy timestamp reads “Scoring time unavailable; a fresh audit may still be subject to service limits.” Markdown carries equivalent plain-language text.
- R24. Staging verification uses one designated safe regression target, waits until its `refresh_after` when necessary, confirms the audit switch and rate-limit budget permit one sanctioned run, captures the fresh terminal result, and immediately repeats every cached read surface without adding a force-refresh bypass.
...
- F1. **Orient from a result page**
  ...
  - **Outcome:** The agent knows the audit state and when cache reuse expires, without treating that time as a promise that a fresh audit will be available.
...
- AE6. Given a fresh completed audit, when persistence and terminal responses finish, then storage and terminal responses use one identical scoring instant, fresh terminal responses report `cached: false`, and subsequent cache reads report `cached: true` with that same instant.
...
- AE9. Given target-controlled evidence that contains instructions or exceeds the remaining WebMCP response budget, when remediation is retrieved, then the complete catalog prompt remains unchanged and the target evidence is not embedded in it.
- AE10. Given more than one page of findings, when pages are requested until `next_offset` is null, then the combined IDs follow R20 exactly with no duplicates or gaps.
...
- KTD1. **Use `origin/dev` as the implementation base in an isolated worktree.** (session-settled: user-directed — chosen over modifying the Sounding worktree: preserves unrelated work and starts from schema 0.4.) Create `feat/web-audit-agent-remediation` and verify its HEAD against `origin/dev` before editing. Implements KD3 and R15.
...
- KTD3. **Centralize selection and prompt eligibility.** (session-settled: user-directed — chosen over a fixed MUST-only workflow: agents need task-specific priority and status selection.) One browser-safe selector owns exact enum validation, OR-within/AND-across filtering, deterministic order, pagination, and the observed-remediable predicate used by the human widget and WebMCP tools. Implements KD2 and R2-R9.
- KTD4. **Paginate whole JSON items under the existing execution cap.** Packing reserves space for pagination metadata and never applies a blind string slice to serialized JSON or a prompt. Build-time validation proves each static prompt fits both the direct and one-item batch envelopes. Implements R8 and R21.
- KTD5. **Keep freshness in an outer envelope.** (session-settled: user-directed — chosen over WebMCP-only metadata: every per-target audit consumer needs to diagnose stale results.) A shared `WebAuditFreshness` shape prevents response drift without changing the persisted scorecard or bumping schema 0.4. Implements KD1 and R10-R14.
...
- KTD9. **Separate target evidence from remediation instructions.** The visible Result field carries escaped target evidence; prompt text contains only site-owned catalog content and stable audit facts. Static direct and single-item batch envelopes must fit `EXECUTE_MAX`, and no runtime evidence can consume that budget. Build validation rejects an oversized catalog prompt so it must be shortened before shipping. Implements R19 and AE9.
- KTD10. **Use one total ordering and pagination contract.** The selector applies R20 before offset pagination and the R21 bounds on every worksheet, summary, batch-remediation, and clipboard selection. Implements R5, R8, R20, and R21.
- KTD11. **Present refresh eligibility without promising availability.** Human and machine surfaces expose the same instant, while copy and documentation state that runtime gates can still prevent a fresh audit. Implements R12, R23, and R24.
...
- **Output-cap corruption:** JSON truncation can make omitted findings unreachable. Mitigate by excluding runtime evidence from prompts, proving direct and one-item batch envelopes fit, and packing whole items with bounded pagination.
- **Prompt injection:** Audited sites control evidence text. Mitigate by keeping that evidence in the escaped Result surface and out of instruction-bearing remediation prompts, with adversarial evidence fixtures.
- **Predicate drift:** Worker remediation, clipboard assembly, and WebMCP can disagree on eligibility. Mitigate with the shared browser-safe selector and cross-surface parity tests.
...
- **Dependencies:** U1.
- **Files:** `src/worker/audit-web/display.ts`, `src/worker/audit-web/route.ts`, `src/worker/audit-web/summary-render.ts`, `tests/web-audit-scorecard-format.test.ts`, `tests/web-audit-routes.test.ts`.
- **Approach:**
  ...
  3. Render the R23 freshness states in HTML and markdown plus one stable page-level machine context for scores, counts, and freshness.
  4. Put canonical `data-keyword`, `data-tier`, `data-status`, and `data-unprobed` attributes on every `.web-check[data-id]` root.
  5. Keep prompt carriers conditional on the existing observed-remediable rule and keep target evidence in the escaped Result field rather than prompt text.
...
- **Test scenarios:**
  ...
  - A cached schema 0.4 row missing keyword and tier receives both values from the live registry before HTML or markdown rendering.
  ...
  - Future, expired, and null timestamp states use the exact R23 meanings, semantic HTML timestamps, equivalent markdown, and screen-reader-readable structure at a narrow viewport.
...
- **Dependencies:** U3.
- **Files:** `src/shared/web-audit-findings.ts`, `src/worker/audit-web/remediation.ts`, `src/client/assemble-prompt.ts`, `src/client/clipboard.ts`, `src/client/webmcp-result.ts`, `src/client/webmcp-lib.ts`, `tests/web-audit-assemble-prompt.test.ts`, `tests/webmcp.test.ts`.
- **Approach:**
  1. Put canonical status values and remediation eligibility in a browser-safe shared module, then generalize the pure finding selector around the R20 ordering and R21 pagination contract.
  ...
  3. Keep target-controlled evidence out of remediation prompts, prove the largest static direct and single-item batch envelopes fit, and pack only complete items under `EXECUTE_MAX`.
...
- **Test scenarios:**
  ...
  - Default and boundary values for offset and limit, offsets beyond total, empty arrays, duplicate filters, terminal null continuation, and cap-shortened pages follow R21.
  - Summary scores and counts equal the rendered audit context; issues include actionable rows and non-remediable `error` rows.
  - Worksheet, individual prompt, batch prompt, and summary responses all mirror the page-level `cached`, `scored_at`, and `refresh_after` values.
  - The largest static prompt fits both direct and one-item batch JSON envelopes, including freshness and pagination metadata, within `EXECUTE_MAX`.
...
- **Approach:**
  1. Document exact filter enums, ordering, pagination states, remediation eligibility, untrusted-evidence separation, and the freshness response envelope.
  ...
  4. After the feature PR reaches `dev`, follow R24 with the designated staging regression target and record the fresh terminal result plus the immediate cached browser, regular MCP, HTML, markdown, and WebMCP reads.
...
- **Test scenarios:**
  ...
  - Published freshness copy defines `refresh_after` as cache-expiry eligibility and names the runtime gates that can still block a fresh audit.
...
- U3 is done when HTML and markdown expose equivalent, accessible refresh-eligibility states and every result row carries registry-hydrated canonical metadata independent of remediation.
- U4 is done when agents can filter all exact schema 0.4 statuses and priorities, retrieve every observed actionable prompt through valid paginated JSON, and obtain a complete audit summary under the WebMCP cap without target evidence entering prompts.
- U5 is done when public docs match the shipped schemas, CI is green, staging serves the new tools and fields, and the R24 fresh-then-cached verification succeeds.
