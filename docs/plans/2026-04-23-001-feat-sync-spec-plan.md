...
date: 2026-04-23
parents:
  - https://github.com/brettdavies/agentnative-spec/blob/dev/docs/plans/2026-04-22-002-post-frontmatter-roadmap.md
...
artifacts from `brettdavies/agentnative-spec` at a pinned SHA / tag:
...
Destination lives under `src/data/spec/` — the site's established home for vendored machine-readable artifacts —
**not** under `content/principles/`. The site's `content/principles/` copy is manually written and explicitly kept
independent of the spec's canonical text per `AGENTS.md`. This plan preserves that separation.

Tracked upstream as item 4 of
[agentnative-spec roadmap 002](https://github.com/brettdavies/agentnative-spec/blob/dev/docs/plans/2026-04-22-002-post-frontmatter-roadmap.md).
...
- **Env var is `SPEC_ROOT` with default `$HOME/dev/agentnative-spec`.** Matches the `ANC_ROOT=$HOME/dev/agentnative`
...
- Header comment template (mirrors `sync-coverage-matrix.sh`'s):
  ...
  #
  # Source of truth: brettdavies/agentnative-spec
...
- **API surface parity:** The shields.io badge endpoint (spec roadmap 002, item 2) and the coupled-release protocol in
  `brettdavies/agentnative-spec:CONTRIBUTING.md` both assume the site can cite a spec version. This plan makes that
  citation mechanical rather than manual.
...
- **Parent roadmap (spec repo):**
  [`2026-04-22-002-post-frontmatter-roadmap.md`](https://github.com/brettdavies/agentnative-spec/blob/dev/docs/plans/2026-04-22-002-post-frontmatter-roadmap.md),
  item 4
...
- Target tag: `v0.2.0` (commit `83bf0fd`) in `brettdavies/agentnative-spec`
- Related repos: `brettdavies/agentnative-spec` (upstream), `brettdavies/agentnative-cli` (sibling commit-a-copy
  consumer — see its own plan at
